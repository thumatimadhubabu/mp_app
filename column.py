...
def extract_column_tables(text):
    rows = text.split('\n')
    num_rows = len(rows)
    table_data = []
    ...
    for row in rows:
        columns = row.split()
        if num_rows > 1:  # Check if there is more than one row in the text
            if len(columns) > 1:  # Consider as a column if there are more than 1 value in a row
                table_data.append(columns)
            else:
                # If a row has only one value, assume it's a heading and remove it from all columns
                for i, col in enumerate(columns):
                    if i < len(table_data):
                        table_data[i].append(col)
...
def ocr_to_excel(image_path, max_pages=None):
    ...
    # Extract column-wise table data from cleaned text
    column_tables_data = extract_column_tables(cleaned_text)

    # Convert the column-wise table data to a DataFrame
    df = pd.DataFrame(column_tables_data)

    # Save the DataFrame to an Excel file
    excel_file_path = image_path.replace(file_extension, '_column_tables.xlsx')
...
def select_files():
    ...
    if file_paths:
        ...
        for file_path in file_paths:
            ...
            print(f"Column-wise table output saved in: {output_path}")
